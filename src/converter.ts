...
const rules: ConversionRule[] = [
  ...
  ...[
    ...
    pattern: new RegExp(`([${KANJI_NUMS_DOT}]+)${unit}`, "g"),
  ...
  {
    pattern: new RegExp(
      `第([${KANJI_NUMS}]+)(?![${KANJI_NUMS}])(?!取得者|債務者|者|方|般)`,
      "g",
    ),
  ...
  {
    pattern: new RegExp(
      `([条項章節款目\\d])((?:の[${KANJI_NUMS}]+)+)(?![${KANJI_NUMS}])(?!方|般)`,
      "g",
    ),
    replace: (_: string, prefix: string, branches: string) =>
      prefix +
      branches.replace(
        new RegExp(`の([${KANJI_NUMS}]+)`, "g"),
        (__: string, n: string) => `の${convertString(n)}`,
      ),
  },
...
function convertString(str: string): string {
  ...
  if (!hasUnits) {
    return str.split("").map((c) =>
      c === "・" ? "." : (japaneseNumeralToArabicNumeral[c] ?? c)
    ).join("");
  } else {
